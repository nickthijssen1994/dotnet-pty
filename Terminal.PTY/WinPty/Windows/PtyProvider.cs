...
namespace Terminal.PTY.WinPty.Windows
{
	...
	internal class PtyProvider : IPtyProvider
	{
		...
		private static void ThrowIfErrorOrNull(string message, IntPtr err, IntPtr ptr)
		{
			...
			if (ptr == IntPtr.Zero) throw new InvalidOperationException(message + ": unexpected null result");
		}
		...
		private static void ThrowIfError(string message, IntPtr error, bool alwaysThrow = false)
		{
			...
			if (alwaysThrow) throw new InvalidOperationException(message);
		}
			...
			CancellationToken cancellationToken)
		{
			...
			{
				var slash3 = pipeName.IndexOf('\\', 2);
				if (slash3 != -1) serverName = pipeName.Substring(2, slash3 - 2);

				var slash4 = pipeName.IndexOf('\\', slash3 + 1);
				if (slash4 != -1) pipeName = pipeName.Substring(slash4 + 1);
			}
		...
		private static string GetAppOnPath(string app, string cwd, IDictionary<string, string> env)
		{
				...
				if (Path.IsPathRooted(app))
				{
					if (isWow64)
					{
						...
						if (app.StartsWith(system32PathWithSlash, StringComparison.OrdinalIgnoreCase))
						{
							...
							if (File.Exists(sysnativeApp)) return sysnativeApp;
						}
					}
			...
			if (isWow64)
			{
				...
				if (indexOfSystem32 >= 0 && indexOfSysnative == -1) paths.Insert(indexOfSystem32, sysnativePath);
			}
			...
			CancellationToken cancellationToken)
		{
			...
			var config = winpty_config_new(WINPTY_FLAG_COLOR_ESCAPES, out error);
			...
			var handle = winpty_open(config, out error);
			...
			var env = GetEnvironmentString(options.Environment);
			...
			var spawnConfig = winpty_spawn_config_new(
			...
			var spawnSuccess = winpty_spawn(handle, spawnConfig, out var hProcess, out var thread, out var procError,
				out error);
			...
			if (!spawnSuccess)
			{
				...
				ThrowIfError("Unable to start WinPTY terminal process", error, true);
			}
			...
			CancellationToken cancellationToken)
		{
			...
			finally
			{
				...
				if (hPC != IntPtr.Zero && hPC != INVALID_HANDLE_VALUE) pseudoConsoleHandle.InitialSetHandle(hPC);
			}
			...
			var lpEnvironment = Marshal.StringToHGlobalUni(GetEnvironmentString(options.Environment));
			try
			{
				var app = GetAppOnPath(options.App, options.Cwd, options.Environment);
				var arguments = options.VerbatimCommandLine
				...
				if (!success)
				{
					...
					throw new InvalidOperationException(
						$"Could not start terminal process {commandLine}: {exception.Message}", exception);
				}
