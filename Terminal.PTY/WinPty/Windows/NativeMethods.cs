...
namespace Terminal.PTY.WinPty.Windows
{
	internal static class NativeMethods
	{
		...
		private static readonly Lazy<bool> IsPseudoConsoleSupportedLazy = new(
			() =>
			{
				var hLibrary = LoadLibraryW("kernel32.dll");
		...
		internal static int CreatePseudoConsole(Coord coord, IntPtr input, IntPtr output, uint flags,
			out IntPtr consoleHandle)
		{
		...
		[ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
		private static extern int CreatePseudoConsole64(Coord coord, IntPtr input, IntPtr output, uint flags,
			out IntPtr consoleHandle);
		...
		[DllImport("conpty.dll", EntryPoint = "CreatePseudoConsole")]
		[ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
		private static extern int CreatePseudoConsole86(Coord coord, IntPtr input, IntPtr output, uint flags,
			out IntPtr consoleHandle);
		...
		internal struct STARTUPINFOEX
		{
			...
			internal void InitAttributeListAttachedToConPTY(SafePseudoConsoleHandle handle)
			{
				...
				var wasInitialized = InitializeProcThreadAttributeList(IntPtr.Zero, AttributeCount, 0, ref size);
				...
				if (!wasInitialized)
					throw new InvalidOperationException("Couldn't create new process attribute list",
						new Win32Exception());
