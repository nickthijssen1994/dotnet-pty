...
namespace Terminal.PTY.WinPty
{
	...
	public static class PtyProvider
	{
		...
		{
			if (string.IsNullOrEmpty(options.App)) throw new ArgumentNullException(nameof(options.App));

			if (string.IsNullOrEmpty(options.Cwd)) throw new ArgumentNullException(nameof(options.Cwd));

			if (options.CommandLine == null) throw new ArgumentNullException(nameof(options.CommandLine));

			if (options.Environment == null) throw new ArgumentNullException(nameof(options.Environment));
